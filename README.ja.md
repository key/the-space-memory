...
インデックス時・クエリ時ともにテキストは Unicode NFC（正準結合）に正規化される
ため、バイト表現が異なる視覚的に同一のテキスト（濁点の合成済み/分解済みなど）
は常に一致する。この正規化は導入以降にのみ適用される — 導入前にインデックス
済みのコンテンツは非正規化テキストのままの場合がある。`tsm rebuild --apply` で
既存コンテンツを再正規化し、続けて `tsm dict import` で `user_dict.simpledic` /
`reject_words.txt` から辞書 verdict を再同期すること。
...
| `TSM_STDERR_CAP_BYTES` | `20000000` | _(なし)_ | `tsmd-stderr.log` のサイズ上限（バイト）。長寿命セッション中にこれを超えると daemon が truncate する（`tsm start` の度にも truncate される）。`0` または数値として解釈できない値は既定値にフォールバック |
