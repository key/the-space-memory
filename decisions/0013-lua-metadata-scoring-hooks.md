...
- **Lua を採用**。一般的で知名度が高く、ファイルを編集して保存すれば即反映される
  （コンパイル工程がない）ため「ユーザー編集可能なスクリプト」要件に合致する。
...
- core の最終スコアは次に置き換える:
  ...
  `time_decay` × `status_penalty` を再現する。
...
- **`extract`**: ファイル across で並列、ファイル内のチェーンは順序実行。
  mlua の `Lua` はスレッド跨ぎで共有できない（`Send` だが `Sync` でない）ため、
...
| いつ | 挙動 |
|---|---|
| **ロード時（`tsm start`）の構文 / コンパイルエラー** | **fail-fast**。daemon を起動させず、該当ファイル名とエラーを表示（ADR-0011 の哲学、ADR-0012 のチャネルで surface） |
| **実行時（特定ファイルの `extract` / 特定ヒットの `score`）エラー** | **fail-safe**。warn ログを出力して stderr / `tsmd.log` に表示（ADR-0012 の②）し、`extract` はその寄与を捨て、`score` は中立の係数 1.0 にフォールバックする。daemon は落とさない |
...
**なぜ WASM ではないか**: WASM はコンパイル済みバイナリであり、編集に
ソース + ツールチェーン + ビルド工程を要する。「編集して即反映」という
ユーザー編集スクリプトの体験と構造的に相容れない。ランタイムも重く、
