...
`tests/search-test-patterns.md` documents a standard query set (`rust`,
`LoRa トラッカー`, `狩猟免許`, etc.) used historically for manual quality
spot-checks. Those queries were written against the live, multi-workspace
content this tool is actually used against day to day — content that has
no equivalent in this repository's test assets. Annotating those queries
with relevant-document IDs would require either shipping a slice of that
...
this harness ships its own self-contained corpus and query set instead.
...
almost exactly one chunk (20 documents → 24 chunks), so the practical
...
| Path | Contents |
|---|---|
| `tests/golden/corpus/` | 20 Markdown documents across 5 topic clusters: `search-engine/` (this repo's own stack — rust, lindera, candle, SQLite), `lora-tracker/` (IoT/telemetry), `hunting/` (射撃/ハンドロード/狩猟), `microfactory/`, and `garden/` (a distractor cluster with no topical overlap, used as a negative control). Two documents (`hunting/season-report-{recent,old}.md`) exist solely to exercise temporal filtering. |
