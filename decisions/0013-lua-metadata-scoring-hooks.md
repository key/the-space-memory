...
- 出力は**任意キーの map**。core は中身の意味を知らず、1 ドキュメント 1 レコードの
  JSON として DB に保存する。
- チェーンの合成は**キー単位のシャローマージ**。各スクリプトの戻り値のうち
  **値があるキー（Lua の `nil` は不在＝キーを返さない）だけ**が累積 map を上書きする。
  返さなかったキーは前段の値が残る。値はスカラ限定（バリデーション節）のため
  ネストの deep-merge は考慮しない。
- 同一キーの衝突は**ファイル名昇順で後勝ち**（数字プレフィックスで優先度を明示）。
  ただし各スクリプトは前段までの累積を `ctx.metadata` で読めるため、後勝ちは
