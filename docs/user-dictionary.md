...
└── user_dict.csv           # lindera に読ませる辞書ファイル（IPAdic 形式）
```
...
- **accepted** --- `dict-update` で辞書 CSV に追加済み
...
| ファイル | 役割 |
|---|---|
| `src/user_dict.rs` | 候補収集（`collect_from_text`）、CSV エクスポート、accept/reject 操作 |
| `src/tokenizer.rs` | lindera の初期化。`user_dict.csv` を読み込んで形態素解析に適用 |
...
3. 既に `user_dict.csv` に存在する語はスキップ
...
- 新規の語を `user_dict.csv` に追記し、`accepted` にマーク
...
`user_dict.csv` をテキストエディタで編集し、不要な行を削除する。
